...
binwidth is the width of each column.
...
#it's like take mysleep and mutate extra_std_tidy into as.numeric(scale(extra))

#read %>% as "and then." can put a bunch of them and they're done in order. is probably best to think of them being done to the firs tthing to the left of the first %>% 
...
```{r}
mysleep$extra_sqrt<-sqrt(abs(mysleep$extra))
mysleep<-mysleep %>% mutate(extra_sqrt = sqrt(abs(extra)))
```
...
mysleep<-mutate(ifelse())

ifelse(first thing is true, do something) +

mysleep$extra_cat<-
  
  mysleep<-mysleep %>% mutate(extra_sqrt = sqrt(abs(extra)))

mysleep<-mysleep %>% mutate(extra_cat = ifelse(extra < 0, "low"), ifelse(extra >=3, "high", "medium")


mysleep<-mysleep %>% mutate(extra_corrected = ifelse(ID=c("1","3","7"), extra+2, extra))
...
std.err<-function(vector) {
  std.dev<-sd(vector)
  std.err<-std.dev/(sqrt(length(vector)))
  return(std.err)
}

std.err2<-function(vector) {
  std.err<-sd(vector)/(sqrt(length(vector)))
  return(std.err)
}
